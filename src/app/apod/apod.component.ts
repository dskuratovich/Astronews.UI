...
export class APODComponent implements OnInit {
  data: BehaviorSubject<ApodModel[]> = new BehaviorSubject<ApodModel[]>([]);
  ...
  constructor(
    ...
    this.date = new Date();
    this.searchService.searchTerm$.subscribe(this.filterData);
  }
  ...
  private filterData(term: string) {
    ...
    if (property) {
      switch (property) {
        ...
        case 'e':
          this.filteredData = this.data.filter((item) =>
            item.explanation.includes(value)
          );
          break;
        case 'c':
          this.filteredData = this.data.filter((item) =>
            item.copyright.includes(value)
          );
          break;
        case 'd':
          this.filteredData = this.data.filter((item) =>
